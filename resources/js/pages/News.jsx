    ...
    return (
        <>
            <section className="relative overflow-hidden bg-forest-950 py-12 text-white sm:py-20">
                ...
                <div className="page-shell relative grid gap-8 sm:gap-9 lg:grid-cols-[1fr_.8fr] lg:items-end" data-reveal>
                    <div>
                        <div className="eyebrow eyebrow-light"><Newspaper size={16} weight="BoldDuotone" /> Local agricultural news</div>
                        <h1 className="mt-6 max-w-3xl font-display text-[2.7rem] font-extrabold leading-[1] tracking-[-0.055em] sm:text-6xl">What is changing around your market?</h1>
                    ...
                    <form onSubmit={submit} className="border border-white/12 bg-white/8 p-2 backdrop-blur">
                        <label className="flex items-center gap-3 bg-white px-3" htmlFor="news-location">
                            ...
                            <button type="submit" className="grid size-11 shrink-0 place-items-center bg-transparent text-forest-950 transition-colors hover:text-forest-600" aria-label="Search news"><Search size={21} weight="BoldDuotone" /></button>
                        </label>
                    </form>
                </div>
            </section>
            ...
            <section className="page-shell py-8 sm:py-14">
                <div className="flex flex-col gap-4 border-b border-forest-950/8 pb-7 sm:flex-row sm:items-end sm:justify-between" data-reveal>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-stone-400">
                        ...
                        <button type="button" onClick={() => loadNews(activeLocation)} className="grid size-11 place-items-center bg-transparent text-forest-700 transition-transform hover:rotate-45" aria-label="Refresh news"><RefreshCw size={18} className={loading ? 'animate-spin' : ''} /></button>
                    </div>
                </div>

                <div className="mt-6 flex items-start gap-3 border-l-2 border-forest-700 bg-sage-50 px-4 py-4 text-sm leading-6 text-stone-600 sm:px-5" data-reveal>
                ...
                {loading ? <NewsSkeleton /> : featured ? (
                    <div className="mt-7 sm:mt-8" data-reveal>
                ...
                <div className="mt-14 sm:mt-16" data-reveal>
        ...
        return (
            <article className="premium-card group overflow-hidden">
                <a href={article.url} target="_blank" rel="noreferrer" className="grid min-h-[380px] lg:grid-cols-[1.12fr_.88fr]">
                    ...
                    <div className="flex flex-col justify-center p-5 sm:p-9 lg:p-11">
                        <p className="text-[0.66rem] font-bold uppercase tracking-[.14em] text-forest-700">Lead report · {formatDate(article.published_at)}</p>
                        <h3 className="mt-4 font-display text-2xl font-extrabold leading-[1.12] tracking-[-0.035em] text-forest-950 sm:text-4xl">{article.title}</h3>
...
function ArticleImage({ article, imageFailed, setImageFailed, featured = false }) {
    return (
        <div className={`relative overflow-hidden bg-sage-100 ${featured ? 'min-h-[230px] sm:min-h-[260px]' : 'aspect-[16/9]'}`}>
